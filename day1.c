...
The second window is marked B (200, 208, 210); its sum is 618. The sum of measurements in the second 
...
int count_increase(FILE* datafile){
    ...
    while (fgets(dataline, XMAX, datafile)!=NULL)
    {
        if (last_depth > 0)
        {
            if (atoi(dataline) > last_depth) countplus++;
        }
        last_depth = atoi(dataline);
        
    }
    return countplus-1;
}

int count_three_increase(FILE* datafile){
    
    char dataline[XMAX];
    int tab_three_last[3] = {-1,-1,-1};
    int previous_sum = 0;
    int current_sum = 0;
	int countplus = 0;
    ...
    while (fgets(dataline, XMAX, datafile)!=NULL)
    {
        previous_sum = tab_three_last[0]+tab_three_last[1]+tab_three_last[2];
        tab_three_last[0] = tab_three_last[1];
        tab_three_last[1] = tab_three_last[2];
        tab_three_last[2] = atoi(dataline);
        current_sum = tab_three_last[0]+tab_three_last[1]+tab_three_last[2];
        if (tab_three_last[0] != -1)
        {
            if (current_sum > previous_sum) countplus++;
        }
    }
    return countplus-1;
}
...
int main(int argc, char *argv[])
{
    ...
    {
        case PART1 :
            printf("the number of increasing depths is : %d\n",count_increase(myfile));
            break;
        case PART2 :
            printf("the number of increasing depths is : %d\n",count_three_increase(myfile));
            break;
    }
